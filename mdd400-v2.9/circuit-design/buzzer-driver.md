...
1. **Default-off at power-up** — Apply VDD with AUDIO_PWM unconnected. Pass if the buzzer emits no tone or chirp during the first 500 ms after VDD comes up.
2. **Single-tone correctness** — Drive AUDIO_PWM with a 2 kHz square wave from firmware. Pass if the buzzer emits an audible tone of recognisable pitch at expected loudness.
3. **Tone sweep across the alert palette** — Drive AUDIO_PWM from 1 kHz to 4 kHz in 100 Hz steps. Pass if all tones are clearly audible without distortion or dropouts; record any unusual resonant peaks or dips for firmware-tone-library tuning.
4. **Default-off recovery** — Drive a continuous tone for 5 s, then release AUDIO_PWM to high-impedance. Pass if the tone stops immediately and no audible artefact persists.
5. **Current draw at full duty** — Drive AUDIO_PWM with a 50 % duty-cycle 2 kHz square wave. Probe VDD-side current. Pass if peak / average current is consistent with the calculated ~313 mA peak / ~156 mA average values.
...
- **Default-off at power-up** — no audible chirp during the first 500 ms after VDD comes up with AUDIO_PWM unconnected.
- **Single-tone correctness** — 2 kHz square wave produces audible tone at expected loudness.
- **Tone-palette sweep** — 1 kHz to 4 kHz in 100 Hz steps; record any resonant peaks / dips for firmware library tuning.
- **Default-off recovery** — tone stops immediately when AUDIO_PWM is released to high-impedance.
- **Current draw at full duty** — peak / average VDD-side current consistent with calculated ~313 mA peak / ~156 mA average.
...
- [ESP32 Module](./esp32-module) — host MCU; AUDIO_PWM is an ESP32 GPIO driven by the LEDC peripheral.
- [Power Monitor](./power-monitor), [Temperature Sensor](./temperature-sensor) — alert sources that the buzzer annunciates.
