...
<SchematicViewer src="/img/schematics/mdd400-v2.9/display_interface_7c449cda.svg" alt="Display Interface schematic — full sheet (display power switch with VDSP rail conditioning; DWIN DMG48480F040 FPC connector). Zoom and pan freely; per-sub-circuit zoomed views appear below." />
...
1. **Default-off at power-up** — Apply VDD and probe VDSP at the J4 connector with DISP_EN unconnected (or firmware-controlled GPIO held high-impedance). Pass if VDSP measures &lt; 50 mV.
2. **Power-on sequence** — Drive DISP_EN HIGH from firmware. Probe Q4 gate. Pass if Q4 gate is pulled to ≤ 0.5 V within one GPIO cycle, confirming Q5 saturates and Q4 enters full enhancement.
3. **VDSP rail under display load** — Measure VDSP at the J4 connector with the display fully active (backlight on, UI animation running). Pass if VDSP stays within ±2 % of the VDD rail value with no individual dip below 4.85 V.
4. **Display operating-current characterisation** — Measure VDD-side current under several display states: backlight off, backlight at 25 / 50 / 75 / 100 %, with and without active UI animation. Record peak. Confirm peak stays well within Q4's 4.2 A and FB3's 3 A ratings. *(Documents the actual current draw — the performance review assumed 400 mA conservatively.)*
5. **Hard-reset recovery** — With the display on, briefly drive DISP_EN LOW for 10 ms then HIGH. Pass if the T5L re-boots and DGUS II commands resume after the documented T5L boot time (~600 ms typ).

---

## DWIN display FPC connector and UART2 link

<SchematicViewer src="/img/schematics/mdd400-v2.9/display_interface_7c449cda.svg" alt="DWIN display FPC connector sub-circuit — J4 (Xunpu FPC-05FB-50PH20 50-pin 0.5 mm FPC connector), carrying VDSP, GNDREF, and the DISP_TX / DISP_RX UART2 link to the display's T5L SoC." initialFocus="19.05 12.7 127.0 88.9" />
...
1. **UART round-trip** — Send a DGUS II "read variable" command at a known address. Pass if the display responds within the documented latency window and the response frame parses cleanly.
2. **Touch event reception** — Tap a documented touch zone. Pass if the corresponding DGUS II touch-event frame arrives on DISP_RX with the correct touch coordinates.
3. **FPC seating** — After assembling the FPC into J4 and engaging the clamshell, gently flex the FPC at the connector edge while monitoring the UART link. Pass if there are no momentary disconnects.
...
1. The ambient light sensor (ALS) lives on the I²C bus — see the [Ambient Light Sensor](./ambient-light-sensor) page for the sensor part and address.
...
- **Default-off at power-up** — VDSP &lt; 50 mV at J4 with DISP_EN unconnected.
- **Power-on transition** — Q4 gate pulled to ≤ 0.5 V within one GPIO cycle after DISP_EN goes HIGH.
- **VDSP rail under display load** — within ±2 % of VDD with no dip below 4.85 V, even under animated UI + 100 % backlight.
- **Operating-current characterisation** — record peak VDD current at several backlight settings; confirm well within Q4 (4.2 A) and FB3 (3 A) ratings. (Performance review used 400 mA as a conservative estimate; replace with measured value.)
- **Hard-reset recovery** — drive DISP_EN LOW for 10 ms, then HIGH; T5L re-boots and DGUS II commands resume.
- **UART round-trip and touch-event reception** — see the per-sub-circuit bring-up lists above.
...
- **C37 dielectric / voltage-derating confirmation** — confirm the Murata "BZ" series code corresponds to a temperature characteristic (X5R vs Y5V / Z5U) where the effective capacitance at 5 V DC bias remains ≥ 4.7 µF. If derating is severe, swap for an X5R-rated 10 µF / 25 V 0805 part.
...
- **DISP_TX / DISP_RX series damping if traces ≥ 50 mm** — already tracked under `## esp32-module` in `v2.10-improvements.md`; cross-referenced from this page because the damping resistors belong adjacent to each transmitter (U3 DISP_TX pad and the display module TX pad respectively).
...
- [ESP32 Module](./esp32-module) — host MCU; DISP_TX / DISP_RX / DISP_EN drive lines.
- [Ambient Light Sensor](./ambient-light-sensor) — ALS used to drive the firmware brightness loop.
