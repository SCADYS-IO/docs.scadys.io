...
The MDD400 has two regulated supplies, generated by two independent **LMR51610 synchronous buck converters** running from the protected bus supply (VSD ≈ 12 V, sourced from the [CAN Bus Power](./can-bus-power.md) section via FB4). They are drawn on the `power_supplies` KiCad sheet:
...
- **VDD 5.0 V** — powers the DWIN capacitive touch display (gated by a high-side switch on the [Display Interface](./display-interface.md) page) and the MLT-8530 buzzer.
...
<SchematicViewer src="/img/schematics/mdd400-v2.9/power_supplies_161c189f.svg" alt="VCC converter (U1) on the right of the Power Supplies sheet — feedback divider R5/R6, feedforward C7, inductor L1, boundary ferrite FB1. Zoom out to see the full sheet." initialFocus="148.59 12.7 135.89 76.2" />
...
<SchematicViewer src="/img/schematics/mdd400-v2.9/power_supplies_161c189f.svg" alt="VDD converter (U6) on the left of the Power Supplies sheet — feedback divider R18/R19, feedforward C23, inductor L2, boundary ferrite FB2. Zoom out to see the full sheet." initialFocus="12.7 12.7 135.89 76.2" />
...
| Consumer | Worst-case current | Notes |
|----------|--------------------|-------|
| DWIN DMG48480F040_01WTC display (via VDSP) | ~250–350 mA | 4.0 " IPS 480×480 + capacitive touch; backlight PWM dominates; gated by Q4 on the [Display Interface](./display-interface.md) page |
...
| FB4 | BLM31KN601SN1L | 1206 ferrite bead — VSC → VSD CAN-domain input filter (covered on the [CAN Bus Power](./can-bus-power.md) page) | [Murata BLM31KN601SN1L](https://www.murata.com/en-us/api/pdfdownloadapi?cate=luBLM&partno=BLM31KN601SN1L) |
...
- [CAN Bus Power](./can-bus-power.md) — derives the protected VSD bus supply (via FB4) that feeds both converter inputs
- [ESP32-S3 Module](./esp32-module.md) — the VCC consumer (U3) and the developer-variant U4/R22 back-up path
- [Display Interface](./display-interface.md) — the VDD high-side switch (Q4) gating the DWIN display backlight load
- [Power Rails](/mdd400/v2.9/quick-reference/power-rails) — the VCC / VDD / VSD rail reference table for the whole board
