...
| Directive / standard | V1.1 status |
| --- | --- |
...
| EU Directives 2011/65/EU + 2015/863/EU — RoHS | Mark present; the BOM is lead-free (the V1.2 InvenTree refresh confirms all components carry "ROHS" suffix; V1.1 BOM is electrically identical) |
...
- **Bill-of-materials lead-free status** — every component on the V1.2 InvenTree refresh (electrically identical to V1.1) carries a `ROHS` suffix. The China-RoHS EFUP variant of the silkscreen mark is used (`SILKS:EFUP_RoHS_China_4`), which is acceptable for both EU and China markets when the BOM is lead-free.
...
6. **QR-code URL correction** — the V1.1 silkscreen carries `docs.scadys.com/mdd400`, a clone-leftover from the project's origin as an MDD400 fork. The V1.2 schematic refresh has the correct URL in place; V1.3 fabrication will be correct.
