...
The chain is designed to deliver:
...
<SchematicViewer src="/img/schematics/canbench-duo-v1.1/lisn_positive_measurement_port_60297cde.svg" alt="LISN+ measurement port schematic (lisn_positive_measurement_port)" initialFocus="31 60 221 70" />

The lower-rail mirror has identical topology with mirrored refdes (e.g. `R20` → `R29`, `C15` → `C20`, `J2` → `J4`):

<SchematicViewer src="/img/schematics/canbench-duo-v1.1/lisn_negative_measurement_port_db5975ce.svg" alt="LISN− measurement port schematic (lisn_negative_measurement_port)" initialFocus="31 60 221 70" />
...
Design intent — calculated in `performance_review/lisn-measurement-ports.md` against V1.1 BOM (electrically identical to V1.2). SPICE simulation against PCB parasitics is pending for full S21 / return-loss verification.
...
- **0.2 mm RF trace widths** — by design (see above). Acceptable at ≤ 108 MHz given the short trace runs; SPICE-confirmed return-loss measurement at the SMA would close this analytically. V1.3 may revisit if HF performance falls short.
- **Sparse SMA ground-via cluster** — fewer ground stitching vias around each SMA than the routing memory recommends. Recurring observation across all three of the SMAs (`J2`, `J4`, and `J6` on the CAN CM port). Either V1.3 layout cleanup or empirical confirmation that the IEC 61000-4-2 ESD performance is acceptable as built.
- **R21 / R30 (5.1 Ω) thick-film** in an otherwise thin-film RF chain — only conducts during clamp events, not under normal operation. V1.3 cleanup candidate.
