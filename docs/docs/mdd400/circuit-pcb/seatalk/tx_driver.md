...
The PNP sources current from `VST` through R52 into the `ST_SIG` line, providing a sharp rising edge to quickly restore the bus to idle. Once C51 charges, the PNP turns off and the line remains passively pulled up.
