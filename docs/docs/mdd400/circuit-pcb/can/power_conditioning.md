...
A 10 µF aluminium electrolytic capacitor is placed after the resettable fuse and before the EMI filter. This capacitor provides damping of high-frequency oscillations on the input cable and absorbs surge energy clamped by the TVS and MOV components. Aluminium electrolytics are specifically chosen for their high effective series resistance (ESR), which provides beneficial resistive damping and suppresses ringing. A 100 kΩ resistor is placed in parallel with this capacitor to ensure safe discharge after power-off.
...
A [P-channel MOSFET](https://lcsc.com/datasheet/lcsc_datasheet_2410121947_Nexperia-PMV240SPR_C5361354.pdf) is used to disconnect the load if the input voltage exceeds a defined threshold. A resistive divider monitors the supply voltage and drives the base of a PNP transistor. When the divided voltage exceeds ~0.65 V, the PNP conducts, pulling up the gate of the MOSFET and turning it off. The threshold is set to approximately 24.6 V, ensuring the [INA219](https://www.ti.com/lit/ds/symlink/ina219.pdf) I2C power sensor (27 V maximum sense voltage) is not overstressed.
...
6. Texas Instruments, [*INA219 Current/Power Monitor With I2C Interface Datasheet*](https://www.ti.com/lit/ds/symlink/ina219.pdf)
