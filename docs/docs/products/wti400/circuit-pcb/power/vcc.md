...
* operate reliably across an 8 V to 14.8 V automotive/RV supply range;
...
![VCC DC-DC schematic](../../assets/images/sch_vcc.png)
...
A 22 µH shielded inductor ([Bourns SRN5040TA-220M](https://www.bourns.com/docs/product-datasheets/srn5040ta.pdf?sfvrsn=df477df6_5)) with 123 mΩ DCR and 1.6 A saturation current is used to balance size, ripple, and thermal performance. Output capacitance consists of two 10 µF X7R MLCCs. A small feed-forward capacitor is fitted (**1 pF** in this revision; footprint allows increase during validation).

Voltage feedback is set via a 100 kΩ / 30.9 kΩ divider.
...
* **provision for an SW-node snubber can be added in a future spin if needed (no footprint in this revision);**
