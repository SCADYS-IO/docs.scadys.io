...
![Status LED circuit](../../images/status_led.png)
...
The control input, labeled `LED_EN`, is connected to a digital output of the ESP32-S3. When the microcontroller drives this pin high (3.3 V), the base-emitter junction is no longer forward biased, and the transistor switches off. This disables the LED. Firmware can use this mechanism to turn off the LED during normal operation, or to flash error codes by toggling the pin.
