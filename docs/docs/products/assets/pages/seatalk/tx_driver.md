...
The EN isolator uses a [TLP2309 opto-isolator](https://lcsc.com/datasheet/lcsc_datasheet_2410010231_TOSHIBA-TLP2309-TPL-E_C85066.pdf) to buffer the `ST_EN` signal across the isolation barrier. This allows the microcontroller to enable or disable the line driver while maintaining galvanic isolation.
...
![SeaTalk\<sup>®\</sup> TX_EN Isolator](../../images/seatalk_en_isolator.png)
...
![SeaTalk\<sup>®\</sup> TX Driver](../../images/seatalk_tx_driver.png)

The TX driver is enabled by a logic-high level on the EN line. When enabled, the output state is controlled by the TX signal (received from the opto-isolated `ST_TX` line).
