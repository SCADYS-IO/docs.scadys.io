...
The power sensor circuit (schematic below) uses a [INA219](https://www.ti.com/lit/ds/symlink/ina219.pdf) Current/Power Monitor with I²C Interface, located in the `CAN` domain. To maintain galvanic isolation from the `DIGITAL` domain, an [ISO1541](https://www.ti.com/lit/ds/symlink/iso1541.pdf) I²C isolator transfers `I2C_SCL` and `I2C_SDA` signals across the galvanic barrier.

The INA219's A1 and A0 address pins are both tied to ground, setting the address of the sensor on the I²C bus to [`0X40`].
...
The I²C isolator supports bidirectional data communication (`I2C_SDA`), with `I2C_SDA` and `I2C_SCL` pull-up resistors on the isolated side to `VDD`. The digital logic domain already includes pull-up resistors to `VCC`.

![I²C Power Sensor](../../images/i2c_isolator.png)
