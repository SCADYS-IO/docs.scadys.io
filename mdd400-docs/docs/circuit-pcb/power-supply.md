...
<table border="1" cellpadding="6" cellspacing="0">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>Survive ISO 7637-2 Pulse 5b (150 V, 400ms exponential decay); limit to &lt; 60 V</td>
    </tr>
    <tr>
      <td>ESD Protection</td>
      <td>Tolerate ±15 kV air discharge per IEC 61000-4-2</td>
    </tr>
    <tr>
    ...
    <tr>
      <td>EMC Filtering</td>
      <td>Suppress conducted emissions above 1 MHz; limit conducted noise to &lt; 100 mV p-p; contain radiated emissions to meet FCC Part 15 and EN 55032 Class B limits</td>
    </tr>
  </tbody>
</table>
...
Load Dump and Surge Clamping Survive [ISO 7637-2](https://www.iso.org/standard/50925.html) Pulse 5b (150 V, 400ms exponential decay); limit to < 60 V

ESD Protection Tolerate ±15 kV air discharge per IEC 61000-4-2[*IEC 61000-4-2:2008, EMC — Part 4-2: ESD immunity test*](https://webstore.iec.ch/en/publication/68954)[2]
...
Both 12 V inputs — [NMEA 2000](https://www.nmea.org/nmea-2000.html) and legacy serial — are protected from reverse polarity using discrete Schottky diodes. 
...
A solder jumper in parallel with the diode is included on the legacy serial interface for development and testing. This allows the interface to be used as a 12 V power source for external equipment if required, while still maintaining galvanic separation from the [NMEA 2000](https://www.nmea.org/nmea-2000.html) backbone.

### ESD and Load Dump

The first line of defence against both electrostatic discharge (ESD) and high-energy surge events is a high-power transient voltage suppressor (TVS) diode. The [SM8S36CA TVS diode](https://www.smc-diodes.com/propdf/SM8S20CA%20THRU%20SM8S43CA%20N2149%20REV.-.pdf) is used at the 12 V input, upstream of the surge stopper MOSFET, to clamp and absorb energy during overvoltage events.
...
![image001](../assets/images/image001.png)
...
![image001](../assets/images/image002.png)
...
The [SM8S36CA](https://www.smc-diodes.com/propdf/SM8S20CA%20THRU%20SM8S43CA%20N2149%20REV.-.pdf) is placed upstream of the MOSFET and begins clamping at 36 V, reaching a maximum clamped voltage of approximately 58 V during surge conditions. Simulation of [ISO 7637-2](https://www.iso.org/standard/50925.html) Pulse 5b (150 V peak, 80 ms exponential decay) using time-series integration shows the diode absorbs approximately 19.3 J of energy. Assuming a conservative thermal capacitance of 0.16 J/°C and ambient temperature of 40 °C, the peak junction temperature is calculated at approximately 148 °C — safely below the device’s 175 °C absolute maximum rating.
...
- a surge above ~180 V sustained for >100 ms, exceeding the [SM8S36CA](https://www.smc-diodes.com/propdf/SM8S20CA%20THRU%20SM8S43CA%20N2149%20REV.-.pdf) energy capacity;
...
To evaluate the robustness of the protection scheme under more severe conditions, time-series simulations were performed for [ISO 7637-2](https://www.iso.org/standard/50925.html) load dump pulses with peak voltages of 150 V, 175 V, 200 V, 225 V, and 250 V, each decaying exponentially over 400 ms. Both the TVS ([SM8S36CA](https://www.smc-diodes.com/propdf/SM8S20CA%20THRU%20SM8S43CA%20N2149%20REV.-.pdf)) and the Schottky diode ([SS34](https://www.onsemi.com/products/discrete-power-modules/schottky-diodes-rectifiers/ss34)) were evaluated. While both were found to operate within their respective SOA at or below 175 V, simulations show that above this level the TVS junction temperature exceeds 175 °C and cumulative energy approaches the device’s failure threshold. The results confirm the selected components are suitable for [ISO 7637-2](https://www.iso.org/standard/50925.html) Pulse 5b (up to 150 V), but operation beyond 175 V is not guaranteed without risk of failure.
...
This filtering approach is consistent with the recommendations of [ISO 11898-2](https://www.iso.org/standard/63648.html) and helps prevent both emissions and susceptibility to EMI propagating through the [NMEA 2000](https://www.nmea.org/nmea-2000.html) network.
...
The 5 V rail is derived from the filtered 12 V input using a buck-mode switching regulator ([TPS54560B-Q1](https://www.ti.com/lit/ds/symlink/tps54560b-q1.pdf)). It provides up to 5 A peak current and serves as the intermediate supply for the 3.3 V LDO as well as directly powering the serial LCD display. The regulator offers:
